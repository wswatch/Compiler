	...
	public String startCode(boolean ForD) {
		...
		if (ForD) {		// if it is a directory
			...
			txt = "@256\n"
				...
				+ "@SP\n"
				+ "M=D\n"
				+ f.call_main(order,"",0);	// call Sys.init
//			txt = f.call_main(order);
		}
		else {
			txt = "";
		}
...
//			System.out.println(text.get(i));
	...
	public String translate(Order order,int lineNum) throws ParseError {
		...
		case CALL:{
			asm=f.call(order,filename,lineNum);
		};break;
		case RETURN:{
...
class Function {
	...
	public String call(Order order,String filename,int lineNum) throws ParseError {		// call 
		String txt = null;
		if (order.arg1 != ARG1.LABEL) {
			...
			txt = call_main(order,filename,lineNum);
		}
		return txt;
	}
	public String call_main(Order order, String filename, int lineNum) {
		String return_sign = filename + lineNum + "_return";
	...
	public String frame(String Kind) {		// *Frame to Kind
		String txt = null;				// Frame = Frame - 1
		txt = "@R13\n"
			+ "AM=M-1\n"
			+ "D=M\n"
			+ "@"+Kind+"\n"
	...
	public String return_main() {
		String txt = null;
			txt = "@LCL\n"	// R13 = LCL, get the place where call f n is
				+ "D=M\n"	// use R13 as temp value Frame
				+ "@R13\n"
				+ "M=D\n"
